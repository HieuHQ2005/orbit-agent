...
def sms():
    try:
        ...
        if not body:
            resp.message("Send a question. Try: 'Focus: 10 paid design partner calls in 7 days'")
            return str(resp)

        # Limit input length to prevent abuse
        if len(body) > 1000:
            resp.message("Message too long. Please keep questions under 1000 characters.")
            return str(resp)
        ...
        if isinstance(result.actions_48h, str):
            ...
            actions_text = "\n- " + "\n- ".join([action.lstrip('123456789. -•') for action in actions[:3]])
        else:
