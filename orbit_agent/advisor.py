...
class HighOrbitAdvice(dspy.Signature):
    ...
    tool_results: str = dspy.InputField(desc="Results from any financial/analytical tools", default="")
    
    advice: str = dspy.OutputField(desc="2-3 paragraphs of specific, actionable advice tailored to their situation")
    actions_48h: str = dspy.OutputField(desc="List 3-5 specific tasks to complete in 48 hours, one per line, no bullet points or numbers")
    metric_to_watch: str = dspy.OutputField(desc="One metric that best predicts progress in 1-2 weeks")
    risks: str = dspy.OutputField(desc="List 3 immediate risks or assumptions to test, one per line, no bullet points or numbers")
...
class HighOrbitAdvisor(dspy.Module):
    ...
    def _call_llm_with_retry(self, module, **kwargs):
        """Call LLM with retry logic and basic latency logging"""
        start = time.time()
        try:
            ...
            try:
                ...
                advisory = getattr(result, "advice", None) if 'result' in locals() else None
                in_len = len(hist) if isinstance(hist, str) else 0
                out_len = len(advisory) if isinstance(advisory, str) else 0
                logger.info(f"LLM call {module.__class__.__name__} took {duration:.0f} ms (in={in_len} chars, out={out_len} chars)")
            except Exception:
    ...
    def _clean_output(self, text: str) -> str:
        ...
        if any(marker in text for marker in ['Context:', 'History:', 'Playbook:', 'Tool Results:', 'Advice:', 'Actions:', 'Metric:', 'Risks:']):
            # Extract the first meaningful content line that doesn't look like metadata
            lines = text.split('\n')
            for line in lines:
                line = line.strip().replace('**', '')
                # Skip metadata headers and empty lines
                if not line or ':' in line[:20] or line.startswith('**') or 'Context' in line or 'History' in line:
                    continue
    ...
    def forward(self, history: list[dict[str, str]], playbook: str, context: str | None = None):
        try:
            ...
            if recent:
                recent_context = "\n\nRecent advice patterns:\n" + \
                    "\n".join([f"- {r['metric_to_watch']}" for r in recent[:2]])
...
def log_advice(history: list[dict[str, str]], result: dspy.Prediction) -> None:
    ...
    try:
        ...
        if isinstance(result.actions_48h, str):
            actions_list = [line.strip().lstrip('123456789. -•') for line in result.actions_48h.split('\n') if line.strip() and not line.strip().startswith('**')]
        else:
            actions_list = result.actions_48h
            
        risks_list = []
        if isinstance(result.risks, str):
            risks_list = [line.strip().lstrip('123456789. -•') for line in result.risks.split('\n') if line.strip() and not line.strip().startswith('**')]
        else:
